...
nstek_hash_table_init()
{
    ...
    for(depth = 0; depth < NSTEK_DEPTH; depth++)
    {
        ...
        {
            printf("[NSTEK ERROR] DEPTH_%.2d [%.8d] Memory allocation failed.\n", depth + 1, NSTEK_DEPTH_LN_CH(depth));
            exit(1);
        }
        else
        {
	...
	while (!force_quit) {
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			for (j = 0; j < nb_rx; j++) {
                ...
                // TUPLES
                tuple.src_addr = ipv4_hdr->src_addr;
                tuple.dst_addr = ipv4_hdr->dst_addr;
